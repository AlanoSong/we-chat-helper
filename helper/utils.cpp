...
)
{
    ...
    WriteProcessMemory(hWechat, pRemotePath, dllPath, (wcslen(dllPath) + 1) * 2, NULL);
    ...
    HANDLE hThread = CreateRemoteThread(hWechat, NULL, 0, pfnLoadLib, pRemotePath, 0, NULL);
    if (!hThread)
    {
...
)
{
    ...
    if (Module32First(hSnapshot, &me32))
    {
        do
        {
            if (_wcsicmp(me32.szModule, dllName) == 0 || _wcsicmp(me32.szExePath, dllName) == 0)
            {
    ...
    HANDLE hThread = CreateRemoteThread(hProcess, NULL, 0, pfnFreeLib, hModule, 0, NULL);
    if (!hThread)
    {
        CloseHandle(hProcess);
        return FALSE;
